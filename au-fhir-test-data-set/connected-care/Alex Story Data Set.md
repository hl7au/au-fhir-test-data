...
A later data pass has since added `identifier` values sourced from the Services Australia / Healthcare Identifiers Service test data (not the placeholder numbers shown in this document's own tables) to the **standalone** connected-care resources: IHI + Medicare Number on `Patient`, HPI-O + ABN on `Organization`, HPI-O on `HealthcareService`, HPI-I + Medicare Prescriber Number + AHPRA registration `qualification` on `Practitioner`, and HPI-I + Medicare Provider Number on `PractitionerRole`. The **embedded** copies of these resource types inside the two AU Patient Summary document Bundles (see below) still carry no `identifier` — they weren't updated in this pass.
...
1. The embedded `Patient` carries no `identifier` — unlike the standalone `Patient-thompson-alex.json`, which now has one (see above), this inline copy wasn't updated — this makes it technically non-conformant with AU PS's strengthened `Patient.identifier` cardinality until it's synced.
