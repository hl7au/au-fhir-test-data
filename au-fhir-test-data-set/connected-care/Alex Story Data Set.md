...
This data set contains IHI, MEDICARE NO, HPI-O, and HPI-I values throughout (patient demographics at the top of every section table, and every "Provider Organisation" / "Clinician" row). **These are placeholder numbers, not valid identifiers**, and were ignored when this data set was first built.

A later data pass has since added `identifier` values sourced from the Services Australia / Healthcare Identifiers Service test data (not the placeholder numbers shown in this document's own tables) to Alex's `Patient`, `Organization`, `HealthcareService`, `Practitioner`, and `PractitionerRole` resources: IHI + Medicare Number on `Patient`, HPI-O + ABN on `Organization`, HPI-O on `HealthcareService`, HPI-I + Medicare Prescriber Number + AHPRA registration `qualification` on `Practitioner`, and HPI-I + Medicare Provider Number on `PractitionerRole`. This covers both the **standalone** connected-care files and the **embedded** copies of these resource types inside the two AU Patient Summary document Bundles (see below) — the two AU PS Patient copies, the GP-visit Bundle's Bathurst Medical Centre/Dr. Lee, and the specialist Bundle's Ashfield Private Clinic/Dr. Chen all now carry matching identifiers.
...
1. The embedded `Patient` now carries an IHI + Medicare Number `identifier`, matching the standalone `Patient-thompson-alex.json` (see above) and satisfying AU PS's strengthened `Patient.identifier` cardinality.
